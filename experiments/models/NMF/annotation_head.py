...
def main(args):
    ...
    annotation_result_dir = os.path.join(script_dir, "annotation-result", str(state_n))
    ...
    for chromosome in index_region:
        for region in index_region[chromosome]:
            ...
            while not success:
                try:
                    child_process = subprocess.Popen(["sbatch", "node.sh", str(child_id), str(state_n), chromosome, str(start_index), str(end_index), str(resolution_size), annotation_result_dir], stdout=subprocess.PIPE)
