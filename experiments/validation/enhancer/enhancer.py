...
exp_id = None
def readin_region():
    with open(configs.filtered_enhancer_file, 'r') as enhancer_f:
        while True:
            line = enhancer_f.readline().strip('\n')
            if not line:
                break
            inf = line.split()
            chromosome = inf[0]
            ...
            if chromosome not in region:
                region[chromosome] = [(start, end, value)]
            else:
                region[chromosome].append((start, end, value))
