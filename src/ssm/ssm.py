...
import sys
class ssm(object):
    def __init__(self, seed=0, E=5, G=100, K=3, lambda_1=0.1, lambda_2=0.1, lambda_3=0.1, positive_state=False, sumone_state=False, positive_em=False, message_passing=True, verbose=False):
        """
        :param E: #assay 
        :param G: #bp
        :param K: #factor
        :param postive_flag: 0:no contraint 1:abs(Y) 2:max(Y,0)
        :param lambda_1: penalization coef of Y
        :param lambda_2: penalization ceof of Theta
        :param lambda_2: whether to update lambda
        """
        np.random.seed(seed)
        self.E = E
        self.G = G
    ...
    def total_error(self):
        ...
        error = 0
        while g < self.G:
            error += np.sum((self.x_m[:, g] - self.theta_m.T * self.y_m[:, g]).T \
                     * (self.x_m[:, g] - self.theta_m.T * self.y_m[:, g]))
            error += np.sum(self.lambda_1 * self.y_m[:, g].T * self.y_m[:, g]) # penalization
          
            if g < self.G - 1:
                error += np.sum((self.y_m[:, g + 1] - self.lambda_m * self.y_m[:, g]).T * (
                        self.y_m[:, g + 1] - self.lambda_m * self.y_m[:, g]))
               
            g += 1
        error += np.sum(self.lambda_2*np.multiply(self.theta_m, self.theta_m))
        error += np.sum(self.lambda_3*np.multiply(self.lambda_m, self.lambda_m))
        return error
    ...
    def get_theta_msg(self):
        ...
        while g < self.G:
            ...
            g += 1
        lhs_m += self.lambda_2 * np.eye(self.K)
    ...
    def forward(self):
        g = 0
        # initialize
        a_m = self.theta_m * self.theta_m.T + self.lambda_1 * np.eye(self.K)
        ...
        while g <= self.G - 2:
            ...
            a_m_next = self.lambda_1 * np.eye(self.K) \
    ...
    def update_y(self):
        ...
        while g < self.G:
            ...
            if self.sumone_state:
                ...
                lhs_m = (-a_m_f - a_m_b + self.theta_m * self.theta_m.T + self.lambda_1*np.eye(self.K))
...
#                 print("[state]before lag:", (-a_m_f - a_m_b + self.theta_m * self.theta_m.T + self.lambda_1*np.eye(self.K)).I * (
#                     1 / 2 * b_m_f + 1 / 2 * b_m_b + self.theta_m * self.x_m[:, g]))
#                 print("[state]after lag:", self.y_m[:, g])
            else:
                new_y_m = (-a_m_f - a_m_b + self.theta_m * self.theta_m.T + self.lambda_1*np.eye(self.K)).I * (
                    1 / 2 * b_m_f + 1 / 2 * b_m_b + self.theta_m * self.x_m[:, g])
    ...
    def update_y_idv(self):
        ...
        while g < self.G:
            if g == 0:
                self.y_m[:, g] = (self.theta_m * self.theta_m.T + self.lambda_m.T * self.lambda_m + (1 + self.lambda_1) * np.eye(self.K)).I \
    ...
    def update_lambda(self, lhs_m, rhs_m):
        self.lambda_m = np.asmatrix(np.linalg.lstsq(lhs_m + self.lambda_3 * np.eye(self.K), rhs_m, rcond=1)[0]).T
...
if __name__ == "__main__":
    from models.state_space_model.ssmKalman import ssmKalman
    seed = 0
    M = 5
    T = 100
    P = 10
    ...
    lambda_1 = 0.1
    # experiment
    model = ssm(seed=seed, E=P, G=T, K=M, lambda_1=lambda_1, 
        lambda_2=lambda_2, lambda_3=lambda_3, message_passing=True, sumone_state=True, positive_state=True, positive_em=True)
